...
select Category, count (*) as jami_maxsulot
from Products
group by Category
...
select Category, avg(Price) as ortalamasi
from Products
...
select Country, count(*) as harbirdavlatdan
from Customers 
group by Country
...
left join Invoices i on o.OrderID = i.InvoiceID
...
select 
year(OrderDate) as yillik,
avg(orderamount) as dsoan
from Orders
...
select year, [Bektemir], [Chilonzor], [Yakkasaroy], [Mirobod], [Yashnobod] 
from city_population
pivot 
(
...
select top 3 CustomerID, sum(TotalAmount) from Invoices
group by CustomerID
...
select 
		ProductName,
		sum(SaleID)  as necha_marta_sotilgani
from Products p
left join Sales s on p.ProductID = s.ProductID
group by ProductName
order by necha_marta_sotilgani desc 
